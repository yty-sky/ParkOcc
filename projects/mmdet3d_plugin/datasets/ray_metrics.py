...
def generate_lidar_short_rays():
    ...
    init_angle = -(math.pi / 2 - math.atan(1))
    end_angle = 0.21
    pitch_angles = np.linspace(init_angle, end_angle, 100)
...
def process_one_sample(sem_pred, lidar_rays, output_origin, instance_pred=None, occ_class_names=None):
    ...
    if sem_pred.shape[0] == 1:
        sem_pred = sem_pred.squeeze(0)
    ...
    offset = torch.Tensor(_pc_range[:3])[None, None, :].cuda()
    scaler = torch.Tensor([_voxel_size] * 3)[None, None, :].cuda()
    ...
    for t in range(T): 
        ...
        # else:
        pred_pcds = torch.cat([pred_label.float().cpu(), pred_dist.cpu()], dim=-1)
...
def process_one_sample_vis(sem_pred, lidar_rays, output_origin, instance_pred=None, occ_class_names=None):
    # lidar origin in ego coordinate
    # lidar_origin = torch.tensor([[[0.9858, 0.0000, 1.8402]]])
    T = output_origin.shape[1]
    pred_pcds_t = []
    ...
    for t in range(T):
        ...
        mask_dist = (pred_dist[:,0] <= 5)
        pred_label = pred_label[mask_dist]
        coord_index = coord_index[mask_dist]

        if instance_pred is not None:
            pred_instance = instance_pred[coord_index[:, 0], coord_index[:, 1], coord_index[:, 2]][:, None]  # [N, 1]
            pred_pcds = torch.cat([pred_label.float(), pred_instance.float(), pred_dist], dim=-1)
        else:
            pred_pcds = torch.cat([coord_index.cpu(), pred_label], dim=-1)

        pred_pcds = pred_pcds.cpu().numpy()
        pred_pcds_t.append(pred_pcds)
...
def calc_rayiou(pcd_pred_list, pcd_gt_list, occ_class_names):
    ...
    for pcd_pred, pcd_gt in zip(pcd_pred_list, pcd_gt_list):
        for j, threshold in enumerate(thresholds):
            ...
            for i, cls in enumerate(occ_class_names):
                ...
                cls_mask_pred = (pcd_pred[:, 0] == (cls_id+1))
                cls_mask_gt = (pcd_gt[:, 0] == (cls_id+1))
    ...
    iou_list = []
    for j, threshold in enumerate(thresholds):
        iou_list.append((tp_cnt[j] / (gt_cnt + pred_cnt - tp_cnt[j])))
...
def main_rayiou(sem_pred_list, sem_gt_list, lidar_origin_list, occ_class_names):
    ...
    for sem_pred, sem_gt, lidar_origins in tqdm(zip(sem_pred_list, sem_gt_list, lidar_origin_list), ncols=50):
        ...
        valid_mask = (pcd_gt[:, 0].astype(np.int32) != 0)
    ...
    table = PrettyTable([
        'Class Names',
        'RayIoU@0.25', 'RayIoU@0.5', 'RayIoU@1'
    ])
    ...
    for i in range(len(occ_class_names)):
        ...
        ], divider=(i == len(occ_class_names) - 1))
...
def vis_rayocc(sem_gt_list, lidar_origin_list, occ_class_names):
    torch.cuda.empty_cache()

    # generate lidar rays
    # lidar_rays = generate_lidar_rays()
    lidar_rays = generate_lidar_short_rays()
    lidar_rays = torch.from_numpy(lidar_rays)

    pcd_gt_list = []
    for sem_gt, lidar_origins in tqdm(zip(sem_gt_list, lidar_origin_list), ncols=50):

        pcd_gt_res = process_one_sample_vis(sem_gt, lidar_rays, lidar_origins, occ_class_names=occ_class_names)

        # evalute on non-free rays
        for pcd_gt in pcd_gt_res:
            valid_mask = (pcd_gt[..., 3].astype(np.int32) != 0)
            pcd_gt = pcd_gt[valid_mask]
            pcd_gt_list.append(pcd_gt)
...
def main_single_rayiou(sem_pred, sem_gt, lidar_origin, occ_class_names):
    torch.cuda.empty_cache()

    # generate lidar rays
    lidar_rays = generate_lidar_rays()
    lidar_rays = torch.from_numpy(lidar_rays).cuda()

    pcd_pred = process_one_sample(sem_pred, lidar_rays, lidar_origin)
    pcd_gt = process_one_sample(sem_gt, lidar_rays, lidar_origin)

    lidar_rays_short = generate_lidar_short_rays()
    lidar_rays_short = torch.from_numpy(lidar_rays_short).cuda()

    lidar_origin_short = torch.tensor(np.zeros((1,1,3))).cuda()
    pcd_pred_short = process_one_sample(sem_pred, lidar_rays_short, lidar_origin_short)
    pcd_gt_short = process_one_sample(sem_gt, lidar_rays_short, lidar_origin_short)
    ...
    valid_mask_short = (pcd_gt_short[:, 0].astype(np.int32) != 0) & (pcd_gt_short[:, 1].astype(np.int32) <= 3)
    pcd_pred_short = pcd_pred_short[valid_mask_short]
    pcd_gt_short = pcd_gt_short[valid_mask_short]

    assert pcd_pred.shape == pcd_gt.shape
    tp_cnt, gt_cnt, pred_cnt = calc_single_rayiou(pcd_pred, pcd_gt, occ_class_names)

    assert pcd_pred_short.shape == pcd_gt_short.shape
    tp_cnt_short, gt_cnt_short, pred_cnt_short = calc_single_rayiou_short(pcd_pred_short, pcd_gt_short, occ_class_names)
...
def calc_single_rayiou(pcd_pred, pcd_gt, occ_class_names):
    ...
    return tp_cnt, gt_cnt, pred_cnt


def calc_single_rayiou_short(pcd_pred, pcd_gt, occ_class_names):
    thresholds = [0.2]
