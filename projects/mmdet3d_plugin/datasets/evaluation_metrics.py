...
from projects.mmdet3d_plugin.datasets.ray_metrics import main_rayiou, vis_rayocc

# distance_table = [80, 160, 240, 320]
# results_all = np.zeros(17)

distance_table = [40, 70, 130, 160]
results_all = np.zeros(6)
...
def evaluation_octree_semantic_park_merge(pred_occ, gt_occ, img_metas, class_num):
    results = []

    volume_gt = torch.zeros((200, 200, 32), dtype=torch.int64)
    volume_gt = volume_gt.cuda()

    octree_gt = Octree(depth=4, pc_range=[-10, -10, -2.6, 10, 10, 0.6], occ_size=[25, 25, 4])
    octree_gt.cuda()
    octree_gt.build_octree_aux(torch.squeeze(gt_occ, 0), img_metas['build_octree'], img_metas['build_octree_up'], 6)

    _, pred_volume = torch.max(torch.softmax(pred_occ[-1], dim=1), dim=1)
    ...
    dist_low2, dist_low1, dist_high1, dist_high2 = distance_table[0], distance_table[1], distance_table[2], distance_table[3]
    mask_A = np.zeros((200, 200, 32), dtype=np.bool)
    mask_B = np.zeros((200, 200, 32), dtype=np.bool)
    mask_A[dist_low1:dist_high1, dist_low1:dist_high1, :] = True
    mask_B[dist_low2:dist_high2, dist_low2:dist_high2, :] = True
    mask_B = mask_B & ~mask_A
    mask_C = ~(mask_A | mask_B)
...
def evaluation_octree_semantic_park(pred_occ, pred_octree, gt_occ, img_metas, class_num):
    results = []
...
def evaluation_semantic_park(pred_occ, pred_octree, gt_occ, img_metas, class_num):
    results = []
