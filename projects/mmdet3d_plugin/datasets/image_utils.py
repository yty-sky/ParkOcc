...
class CamModel:
    ...
    def get_cam_model(self):
        ...
        with open(self.filename) as f:
            ...
            self.pol = [float(d) for d in data[1:]]  # 多项式

            l = lines[6]
            data = l.split()
            self.length_invpol = int(data[0])        # 逆？ 是干什么用的
            self.invpol = [float(d) for d in data[1:]]

            l = lines[10]
            data = l.split()
            self.xc = float(data[0])                 # 图像中心
...
class Image:
    ...
    def norm2image(self):
        ...
        self.eqr_coord = np.vstack((u, v))  # 像素坐标
    ...
    def sphere2equirect(self):
        ...
        long = np.arctan2(Py, Px)   # Py 与 Px 形成夹角
        lat = np.arctan2(Pz, np.linalg.norm([Px, Py], axis=0))   # PZ 与 [Px, Py] 形成夹角

        # Normalized equirectangular coordinates
        x = - long / np.pi   # [-1, 1]
        # y = 2 * lat / np.pi
        y = 2 * 2 * lat / np.pi# [-1, 1]
        self.norm_coord = np.vstack((x, y))   # 归一化坐标
    ...
    def sphere2fisheye(self):
        ...
        # Calculate the projection on the fisheye image using the camera model   sensor plane -> image plane
        self.world2cam(P)  # [rows, col]

    def world2cam(self, points3D):
        """ Projects 3D points on the image and returns the pixel coordinates. """

        # 只取相机正前方，因为在back相机的测试时发现，后方点云也可能会投影到前视上
        points3D_z_down = points3D[2, :] < 0
        points3D = points3D[:, points3D_z_down]
        self.points_values = self.points_values[points3D_z_down]
        ...
        for k in range(np.shape(points3D)[1]):
            if norm[k] != 0:
                ...
                x = points3D[0][k] * invnorm * rho # 投影到x方向
                y = points3D[1][k] * invnorm * rho # 投影到y方向
    ...
    def cam2world(self, points2D):
        ...
        for k in range(np.shape(points2D)[1]):   # image plane -> camera plane
    ...
    def select_project(self, semantic):
        filtered_semantic_index = self.points_values == semantic
        self.spherical_proj = self.spherical_proj[:, filtered_semantic_index]
        self.points_values = self.points_values[filtered_semantic_index]
